...
function welcomeScreen() {
  ...
  textSize(cnvDimension/13);
  textAlign(CENTER, CENTER);
  text("A CLS Hybrid Instrument", width/2, height/10 * 2);
  textSize(cnvDimension/20);
  text( "Containing playing by Becky Knight, Ann Lovatt and Chris Rawley", width/10, height/10, (width/10) * 8, (height/10) * 8);
...
function startAudio() {
  ...
  Tone.Transport.scheduleRepeat(play_ = () => {player1.start();}, '1m');
  Tone.Transport.scheduleRepeat(play_ = () => {player2.start();}, '1m');
  Tone.Transport.scheduleRepeat(play_ = () => {player3.start();}, '1m');
  Tone.Transport.scheduleRepeat(play_ = () => {player4.start();}, '1m');
  Tone.Transport.scheduleRepeat(play_ = () => {player5.start();}, '1m');
  Tone.Transport.scheduleRepeat(play_ = () => {player6.start();}, '1m');
  retrieveSavedWork();
}
